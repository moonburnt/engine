...
// E.g: TiledMapBase, TiledMap (one layer), TiledMapStatic (the same but you can't
// move or delete objects, just these are stored directly on grid) and reworked
// Fortune Crawler's GameMap (TiledMapDeep)
...
public:
    ...
    void delete_object(int grid_index, bool delete_from_storage) {
    ...
    void place_or_replace(size_t grid_index, int object_id, bool delete_from_storage) {
        if (grid[grid_index] != placeholder_id) {
            delete_object(grid_index, delete_from_storage);
        }
    ...
    std::optional<Point> find_object(int object_id) {
